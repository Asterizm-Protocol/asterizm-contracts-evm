...
contract AsterizmInitializerV2 is UUPSUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable, IInitializerSenderV2, IInitializerReceiver, AsterizmEnv, IAsterizmConfigEnv {
    ...
    /// Initiate asterizm transfer (verstion 1)
    ...
    /// Initiate asterizm transfer (verstion 2, with external relays)
    /// Only clients can call this method
    ...
    function initTransferV2(IzIninTransferV2RequestDto calldata _dto) external payable {
        ...
        if (
            ...
            if (configDto.externalRelayExists) {
                ...
                translatorLib.logExternalMessage{value: configDto.systemFee}(dto);
    ...
    function resendTransferV2(bytes32 _transferHash, address _relay) external payable onlyExistsIngoingTransfer(_transferHash) {
        if (
            _relay != address(0) &&
            address(configLib) != address(0) &&
