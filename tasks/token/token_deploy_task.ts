...
task("token:deploy", "Deploy Multichain token contract")
    ...
    .setAction(async (taskArgs, hre) => {
        ...
        const token = await Token.deploy(initializer.address, BigNumber.from(taskArgs.initSupply), gasPrice > 0 ? {gasPrice: gasPrice} : {});
