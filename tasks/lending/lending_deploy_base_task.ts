...
task("lending:deploy-base", "Deploy base Lending contracts")
    ...
    .setAction(async (taskArgs, hre) => {
        ...
        const Lend = await ethers.getContractFactory("LendingBase");
        const Pool = await ethers.getContractFactory("LendingPool");
        console.log("Deploying base lending contract...");
        const lend = await Lend.deploy(await initializer.getAddress(), taskArgs.feeCollectorAddress, gasPrice > 0 ? {gasPrice: gasPrice} : {});
        ...
        const pool = await Pool.deploy(await lend.getAddress(), taskArgs.poolTokenAddress, gasPrice > 0 ? {gasPrice: gasPrice} : {});
