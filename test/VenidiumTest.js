...
describe("Venidium logic", function () {
    ...
    it("Should stake tokens and send coins, than stake coins and send tokens with native logic and fees", async function () {
        ...
        await expect(translator2.transferMessage(300000, packetValue))
            .to.emit(tokenNativeDst, 'PayloadReceivedEvent');
        await expect(tokenNativeDst.asterizmClReceive(currentChainIds[0], tokenNativeSrc.address, decodedValue[4], decodedValue[6], payload)).to.not.reverted;
        expect(await ethers.provider.getBalance(user.address)).to.equal(startUserNativeBalance.add(resultValueWithDecimals));
        ...
        await expect(translator1.transferMessage(300000, packetValue))
            .to.emit(tokenNativeSrc, 'PayloadReceivedEvent');
        await expect(tokenNativeSrc.asterizmClReceive(currentChainIds[1], tokenNativeDst.address, decodedValue[4], decodedValue[6], payload)).to.not.reverted;
        expect(await token1.balanceOf(tokenNativeSrc.address)).to.equal(startContractTokenBalance.sub(resultValueWithDecimals));
        expect(await token1.balanceOf(user.address)).to.equal(startUserTokenBalance.add(resultValueWithDecimals));
    });
    ...
    it("Should stake tokens and mint tokens, than burn tokens and send tokens with stable logic and fees", async function () {
        ...
        await expect(translator2.transferMessage(300000, packetValue))
            .to.emit(tokenStableDst, 'PayloadReceivedEvent');
        await expect(tokenStableDst.asterizmClReceive(currentChainIds[0], tokenStableSrc.address, decodedValue[4], decodedValue[6], payload)).to.not.reverted;
        expect(await tokenStableDst.balanceOf(owner.address)).to.equal(resultValueWithDecimals);
        expect(await tokenStableDst.totalSupply()).to.equal(resultValueWithDecimals);
        ...
        await expect(translator1.transferMessage(300000, packetValue))
            .to.emit(tokenStableSrc, 'PayloadReceivedEvent');
        await expect(tokenStableSrc.asterizmClReceive(currentChainIds[1], tokenStableDst.address, decodedValue[4], decodedValue[6], payload)).to.not.reverted;
        expect(await token2.balanceOf(tokenStableSrc.address)).to.equal(startContractStableTokenBalance.sub(resultValueWithDecimals));
        expect(await token2.balanceOf(owner.address)).to.equal(startOwnerStableTokenBalance.add(resultValueWithDecimals));
    });
